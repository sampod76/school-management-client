...
const StudentApprove = () => {
  ...
  const {
    ...
    refetch,
  } = useGetSinglePendingStudentQuery(studentId, { skip: !studentId });
  const [
  ...
  const [
    updatePendingStudentData,
    {
      error: updatePendingStudentError,
      isLoading: updatePendingStudentLoading,
    },
  ] = usePatchPendingStudentMutation();
  ...
  // only change value get by the form
        ...
        if (result.isConfirmed) {
          updatePendingStudentApproved({
            ...
            if (result?.data?.success) {
              ...
              router.push(
                "/supper-admin/student-information/student-admission"
              );
            } else {
              Error_model({
                message:
  ...
  const onFinish = async (value) => {
    ...
    setLoading(true);
    updatePendingStudentData({ id: studentId, data: transperdata })
      .then((res) => {
        if (res?.data?.success) {
          setLoading(false);
          refetch();
          Success_model({
            message: "রেজিস্ট্রেশন ফর্ম আপডেট সম্পন্ন হয়েছে",
          });
          //after updating then confirming registration
          handleConfirmAdmintion(res?.data?.data?._id, res?.data?.data?.userId);
        } else {
          Error_model({
            message: res?.error?.data?.message || "ভুল হচ্ছে দয়া করে চেক করুন",
            error: res,
          });
          setLoading(false);
        }
  ...
  if (
    ...
    Error_model({
      message:
        error?.message ||
        updatePendingStudentError?.message ||
        studentGetError?.message ||
  ...
  if (isLoading || studentGetLoading) {
    return <DeshbordLoading />;
  }

  // if (!studentGetLoading && !studentData?.data?._id) {
  //   return <DataNotFound/>
  // }
  return (
    <div /* className="container mx-auto p-5 " */>
      <Form
        initialValues={{
          ...
          // "student.date_of_birth":moment(studentData?.data?.student?.date_of_birth,'YYYY-MM-DD'),
          //
          ...
          // "mother_info.date_of_birth": moment(
      ...
      >
        ...
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} sm={12} md={8}>
            ...
            <Form.Item
              ...
              //   defaultValue={studentData?.data?.mother_info?.date_of_birth}
          ...
          <Col xs={24} sm={12} md={8}>
            ...
            <Form.Item
              ...
              //   defaultValue={studentData?.data?.mother_info?.year_of_death}
            >
              <DatePicker
                defaultValue={dayjs(
                  studentData?.data?.mother_info?.year_of_death,
        ...
        <Form.Item style={{ width: "100%" }}>
          <div className="flex justify-center items-center">
            {loading ||
            updatePendingApprovedLoading ||
            updatePendingStudentLoading ? (
              <Button
                type="default"
                htmlType="submit"
      ...
      <Modal
        ...
        cancelText="Cancel"
      >
        আপনি কি শিক্ষার্থীর ভর্তির আবেদন নিশ্চিত করতে চান?
      </Modal>
    </div>
  );
};
