...
import { usePostStudentMutation } from "@/redux/features/student/studentApi";
...
const AddStudentPage = () => {
  ...
  //
  const [createStudent, { isLoading: studentLoading, error: studentError }] =
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      const result = await axios.post(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/pending-students`,
        transperdata
      );
      if (result?.data?.success) {
        ...
        Success_model({ message: "আবেদনটি সফলভাবে নেয়া হয়েছে" });
      } else {
      ...
      <Form form={form} onFinish={onFinish}>
        ...
        <section>
          <Row gutter={[16, 16]}>
            ...
            <Col xs={24} sm={12} md={8}>
              <Typography.Text>
              বাবার নাম (ইংরেজি) <span className="text-red-500">*</span>
              </Typography.Text>
              <Form.Item
        ...
        {classLoading ? (
          <Space align="center">
            <Spin tip={"দয়া করে বইয়ের লিস্টের জন্য অপেক্ষা করুন"} />
          </Space>
        ) : (
          <section
            style={{
        ...
        <Form.Item labelAlign="top" className="mt-2 ">
          ...
          <div className="relative mb-4 mt-2 md:mt-8 grid grid-cols-1 md:grid-cols-7 gap-x-3">
            ...
            <input
              ...
              className="w-80 bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-[2px] px-3 leading-8 transition-colors duration-200 ease-in-out"
            />
          </div>
          {/* <Form.Item
            name="student.photo"
